    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            if reply_content["completion_tokens"] == 0 and len(reply_content["content"]) > 0:
                ...
                if not reply_content.get("approval_required"):
                    self.sessions.session_reply(reply_content["content"], session_id, reply_content["total_tokens"])
                reply = Reply(ReplyType.TEXT, reply_content["content"])
                if reply_content.get("approval_required") and reply_content.get("approval_id"):
                    reply.approval_required = True
                    reply.approval_id = reply_content["approval_id"]
            else:
                reply = Reply(ReplyType.ERROR, reply_content["content"])
    ...
    def reply_text(self, session: ChatGPTSession, api_key=None, args=None, retry_count=0) -> dict:
        ...
        try:
            ...
            if session.session_id:
                # Forward the app-level session identifier so OpenAI-compatible
                # backends such as CodexBridge can keep a stable server-side thread.
                request_args["user"] = str(session.session_id)
            ...
            approval_id = None
            if content:
                match = re.search(r"\[审批请求\s+#([0-9a-fA-F-]+)\]", content)
                if match:
                    approval_required = True
                    approval_id = match.group(1)
            # logger.debug("[CHATGPT] response={}".format(response))
            logger.info("[ChatGPT] reply={}, total_tokens={}".format(content, response["usage"]["total_tokens"]))
            return {
