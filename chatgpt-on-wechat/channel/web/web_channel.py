...
class WebChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        self.terminal_sessions = {}  # term_session_id -> {proc, queue, cwd}
        self._terminal_lock = threading.Lock()

    def _resolve_codex_workdir(self) -> str:
        """Prefer CodexBridge runtime workingDirectory; fallback to workspace."""
        ...
        try:
            ...
            req = urllib.request.Request(
                f'{self._codex_bridge_root_url()}/v1/runtime-config',
                headers=self._codex_bridge_headers()
            )
            with urllib.request.urlopen(req, context=ctx, timeout=3) as response:
                data = json.loads(response.read().decode('utf-8'))
            runtime = data.get("runtime", data)
            workdir = runtime.get("workingDirectory")
    ...
    def _codex_bridge_root_url(self) -> str:
        base = str(conf().get("open_ai_api_base", "http://127.0.0.1:8080/v1") or "").strip()
        if not base:
            base = "http://127.0.0.1:8080/v1"
        root = base.rstrip("/")
        return root[:-3] if root.lower().endswith("/v1") else root

    def _codex_bridge_headers(self) -> dict:
        api_key = str(conf().get("open_ai_api_key", "") or "").strip()
        return {"Authorization": f"Bearer {api_key}"} if api_key else {}

    def _web_terminal_enabled(self) -> bool:
        return bool(conf().get("web_terminal_enabled", False))

    def _push_terminal_event(self, term_session_id: str, payload: dict):
        with self._terminal_lock:
            session = self.terminal_sessions.get(term_session_id)
        if not session:
            return
        session["queue"].put(payload)

    def _spawn_terminal_reader(self, term_session_id: str):
        def _reader():
            with self._terminal_lock:
                session = self.terminal_sessions.get(term_session_id)
            if not session:
            ...
            try:
                while True:
                    ...
                    if not chunk:
                        time.sleep(0.01)
                        continue
                    self._push_terminal_event(term_session_id, {"type": "output", "content": chunk})
                exit_code = proc.exitstatus
                self._push_terminal_event(term_session_id, {"type": "exit", "code": exit_code})
            except Exception as e:
                self._push_terminal_event(term_session_id, {"type": "error", "message": str(e)})
        threading.Thread(target=_reader, daemon=True).start()

    def start_terminal_session(self, mode: str = "tty"):
        if not self._web_terminal_enabled():
            raise RuntimeError("Web terminal is disabled. Set web_terminal_enabled=true to enable it.")
        if PtyProcess is None:
            raise RuntimeError("pywinpty is not installed in runtime environment")
        term_session_id = str(uuid.uuid4())
        cwd = self._resolve_codex_workdir()
        q = Queue()
        env = dict(os.environ)
        if mode == "plain":
            env["TERM"] = "dumb"
        ...
        env["CLICOLOR"] = "0"
        env["FORCE_COLOR"] = "0"
        proc = PtyProcess.spawn("cmd.exe", cwd=cwd, env=env)
        with self._terminal_lock:
            self.terminal_sessions[term_session_id] = {"proc": proc, "queue": q, "cwd": cwd}
        q.put({"type": "started", "session_id": term_session_id, "cwd": cwd, "command": "codex"})
        self._spawn_terminal_reader(term_session_id)
        # Start Codex in current working directory immediately.
        if mode == "plain":
            proc.write("set TERM=dumb\r\n")
        proc.write("codex\r\n")
        return {"session_id": term_session_id, "cwd": cwd, "command": "codex"}

    def stream_terminal_session(self, term_session_id: str):
        with self._terminal_lock:
            session = self.terminal_sessions.get(term_session_id)
        if not session:
            yield b"data: {\"type\":\"error\",\"message\":\"invalid terminal session\"}\n\n"
            return
        q = session["queue"]
    ...
    def send_terminal_input(self, term_session_id: str, content: str):
        with self._terminal_lock:
            session = self.terminal_sessions.get(term_session_id)
        if not session:
            return {"status": "error", "message": "invalid terminal session"}
        proc = session["proc"]
        if not proc.isalive():
            return {"status": "error", "message": "terminal process already exited"}
        try:
            if content is None:
                content = ""
            raw = False
            if isinstance(content, dict):
                raw = bool(content.get("raw", False))
                content = str(content.get("data", ""))
            if raw:
                proc.write(content)
            else:
                proc.write(str(content) + "\r\n")
            return {"status": "success"}
        except Exception as e:
            return {"status": "error", "message": str(e)}

    def stop_terminal_session(self, term_session_id: str):
        with self._terminal_lock:
            session = self.terminal_sessions.pop(term_session_id, None)
        if not session:
            return {"status": "success"}
        proc = session["proc"]
        try:
    ...
    def startup(self):
        ...
        urls = (
            ...
            '/api/terminal/start', 'TerminalStartHandler',
            '/api/terminal/stream', 'TerminalStreamHandler',
            '/api/terminal/input', 'TerminalInputHandler',
            '/api/terminal/stop', 'TerminalStopHandler',
            ...
            '/api/runtime-config', 'RuntimeConfigHandler',
            '/api/cli-stream', 'CLISSEHandler',
    ...
    def stop(self):
        with self._terminal_lock:
            all_ids = list(self.terminal_sessions.keys())
        for sid in all_ids:
            try:
                self.stop_terminal_session(sid)
            except Exception:
                pass
...
class TerminalStartHandler:
    def POST(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            payload = json.loads(web.data() or "{}")
            mode = str(payload.get("mode", "tty")).strip().lower()
            if mode not in ("tty", "plain"):
                mode = "tty"
            result = WebChannel().start_terminal_session(mode=mode)
            return json.dumps({"status": "success", **result}, ensure_ascii=False)
        except Exception as e:
            logger.error(f"[WebChannel] start terminal error: {e}", exc_info=True)
...
class TerminalStreamHandler:
    def GET(self):
        ...
        term_session_id = params.session_id
        if not term_session_id:
            raise web.badrequest()
        web.header('Content-Type', 'text/event-stream; charset=utf-8')
        web.header('Cache-Control', 'no-cache')
        web.header('X-Accel-Buffering', 'no')
        web.header('Access-Control-Allow-Origin', '*')
        return WebChannel().stream_terminal_session(term_session_id)
...
class TerminalInputHandler:
    def POST(self):
        ...
        try:
            payload = json.loads(web.data() or "{}")
            term_session_id = payload.get("session_id", "")
            content = payload.get("content", "")
            raw = bool(payload.get("raw", False))
            if not term_session_id:
                return json.dumps({"status": "error", "message": "session_id required"})
            result = WebChannel().send_terminal_input(
                term_session_id,
...
class TerminalStopHandler:
    def POST(self):
        ...
        try:
            ...
            if not term_session_id:
                return json.dumps({"status": "error", "message": "session_id required"})
            result = WebChannel().stop_terminal_session(term_session_id)
            return json.dumps(result, ensure_ascii=False)
        except Exception as e:
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
...
class ModelsHandler:
    """Proxy to CodexBridge for available models"""
    def GET(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            import urllib.request
            ...
            req = urllib.request.Request(
                f'{WebChannel()._codex_bridge_root_url()}/v1/models',
                headers=WebChannel()._codex_bridge_headers()
            )
...
class RuntimeConfigHandler:
    """Proxy runtime Codex config read/write to CodexBridge"""
    def GET(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            import urllib.request
            ...
            req = urllib.request.Request(
                f'{WebChannel()._codex_bridge_root_url()}/v1/runtime-config',
                headers=WebChannel()._codex_bridge_headers()
            )
    ...
    def POST(self):
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                data=payload,
                headers={
                    **WebChannel()._codex_bridge_headers(),
...
class CLISSEHandler:
    """Proxy SSE stream from CodexBridge for real-time CLI output"""
    def GET(self):
        web.header('Content-Type', 'text/event-stream; charset=utf-8')
        web.header('Cache-Control', 'no-cache')
        ...
        try:
            ...
            req = urllib.request.Request(
                f'{WebChannel()._codex_bridge_root_url()}/cli-stream',
                headers=WebChannel()._codex_bridge_headers()
            )
            
            # Stream SSE line-by-line to avoid 1024-byte buffering delay.
            with urllib.request.urlopen(req, context=ctx, timeout=3600) as response:
                while True:
                    try:
            ...
            payload = f"data: {json.dumps({'type': 'error', 'message': str(e)})}\n\n".encode("utf-8")
            yield payload
