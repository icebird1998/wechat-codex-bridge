...
const VIEW_META = {
    ...
    'codex-cli': { group: 'nav_monitor', pageText: 'Codex CLI' },
...
function navigateTo(viewId) {
    ...
    document.getElementById('breadcrumb-page').textContent = meta.pageText || t(meta.page);
    if (meta.page) document.getElementById('breadcrumb-page').dataset.i18n = meta.page;
    else delete document.getElementById('breadcrumb-page').dataset.i18n;
    currentView = viewId;
...
// (removed dead code: buildChatMirrorPayload, mirrorChatInputToCli)

function mirrorChatEventToCli(item, requestId) {
    if (!item || !item.type) return;
    const rid = requestId ? String(requestId).slice(0, 8) : 'unknown';
    if (item.type === 'phase') {
        addCLILine(`[chat:${rid}] phase: ${String(item.content || '')}`, 'event');
        return;
    }
    if (item.type === 'tool_start') {
        addCLILine(`[chat:${rid}] tool start: ${item.tool || 'tool'}`, 'event');
        return;
    }
    if (item.type === 'tool_end') {
        const status = item.status || 'success';
        const cost = item.execution_time !== undefined ? ` (${item.execution_time}s)` : '';
        addCLILine(`[chat:${rid}] tool end: ${item.tool || 'tool'} -> ${status}${cost}`, status === 'success' ? 'success' : 'error');
        return;
    }
    if (item.type === 'done') {
        const txt = String(item.content || '').trim();
        if (txt) addCLILine(`[chat:${rid}] done: ${txt}`, 'qq-out');
        else addCLILine(`[chat:${rid}] done`, 'qq-out');
        return;
    }
    if (item.type === 'error') {
        addCLILine(`[chat:${rid}] error: ${String(item.message || 'unknown error')}`, 'error');
    }
}
...
function startSSE(requestId, loadingEl, timestamp) {
    ...
    es.onmessage = function(e) {
        ...
        mirrorChatEventToCli(item, requestId);
    ...
    es.onerror = function() {
        es.close();
        delete activeStreams[requestId];
        addCLILine(`[chat:${String(requestId).slice(0, 8)}] stream disconnected`, 'warning');
        if (loadingEl) { loadingEl.remove(); loadingEl = null; }
        if (!botEl) {
            addBotMessage(t('error_send'), new Date());
...
// Codex CLI Monitor
// =====================================================================
let cliTerminalSessionId = null;
let cliTerminalEventSource = null;
let cliBuffer = [];
let currentLine = null;
let cliMonitorInitialized = false;
let cliRuntimeConfigCache = null;
let cliRequestInFlight = false;
let cliXterm = null;
let cliPendingRawInput = '';
let cliInputFlushTimer = null;
let cliXtermReady = false;
let cliBridgeEventSource = null;
let cliBridgeReconnectTimer = null;
let cliBridgeCandidates = [];
let cliBridgeCandidateIndex = 0;
const MAX_CLI_LINES = 700;

function startCodexCLIMonitor() {
    initCLITerminal();
    updateCLISessionLabel('codexbridge');
    connectCliBridgeStream();
}

function stopCodexCLIMonitor() {
    if (cliTerminalEventSource) {
        cliTerminalEventSource.close();
        cliTerminalEventSource = null;
    }
    // Keep terminal process alive across page switches.
    // This allows chat-panel input to continue mirroring to the same CLI session.
    cliRequestInFlight = false;
    currentLine = null;
    if (cliInputFlushTimer) {
        clearTimeout(cliInputFlushTimer);
        cliInputFlushTimer = null;
    }
    cliPendingRawInput = '';
    if (cliXterm) {
        try { cliXterm.dispose(); } catch (_) {}
        cliXterm = null;
    }
    if (cliBridgeReconnectTimer) {
        clearTimeout(cliBridgeReconnectTimer);
        cliBridgeReconnectTimer = null;
    }
    if (cliBridgeEventSource) {
        cliBridgeEventSource.close();
        cliBridgeEventSource = null;
    }
    cliXtermReady = false;
    updateCLIStatus('Disconnected', 'error');
    updateCLISessionLabel(cliTerminalSessionId);
}

function connectCliBridgeStream() {
    if (cliBridgeEventSource) return;
    if (!cliBridgeCandidates.length) {
        cliBridgeCandidates = getCliBridgeCandidates();
        cliBridgeCandidateIndex = 0;
    }
    const targetUrl = cliBridgeCandidates[cliBridgeCandidateIndex] || '/api/cli-stream';
    try {
        cliBridgeEventSource = new EventSource(targetUrl);
        cliBridgeEventSource.onopen = () => {
            updateCLIStatus('Connected', 'success');
            addCLILine(`[bridge] connected: ${targetUrl}`, 'success');
            fetchRuntimeConfig(true).catch(() => {});
        };
        cliBridgeEventSource.onmessage = (event) => {
            let data;
            try { data = JSON.parse(event.data); } catch (_) { return; }
            handleBridgeCliEvent(data);
        };
        cliBridgeEventSource.onerror = () => {
            if (cliBridgeEventSource) {
                cliBridgeEventSource.close();
                cliBridgeEventSource = null;
            }
            addCLILine('[bridge] disconnected, retrying...', 'warning');
            if (cliBridgeCandidates.length > 1) {
                cliBridgeCandidateIndex = (cliBridgeCandidateIndex + 1) % cliBridgeCandidates.length;
            }
            if (cliBridgeReconnectTimer) clearTimeout(cliBridgeReconnectTimer);
            cliBridgeReconnectTimer = setTimeout(() => {
                cliBridgeReconnectTimer = null;
                connectCliBridgeStream();
            }, 1500);
        };
    } catch (err) {
        addCLILine('[bridge] connect failed: ' + (err?.message || 'unknown'), 'error');
    }
}

function getCliBridgeCandidates() {
    return ['/api/cli-stream'];
}

function handleBridgeCliEvent(data) {
    ...
    if (type === 'session.started') {
        ...
        const sid = data?.sessionId ? String(data.sessionId).slice(0, 12) : 'ephemeral';
        addCLILine('', 'plain');
        addCLILine('━━━━━━━━━━ 新请求 ━━━━━━━━━━', 'header');
        addCLILine(`📥 问题进入  会话=${sid}  模型=${model}`, 'qq-in');
        if (data?.prompt) {
            addCLILine(`   "${truncStr(String(data.prompt), 200)}"`, 'qq-in');
        } else if (data?.promptPreview) {
            addCLILine(`   "${String(data.promptPreview)}"`, 'qq-in');
        }
        addCLILine('', 'plain');
        return;
    }
    ...
    if (type === 'codex.delta') {
        appendCLIText(String(data?.content || ''));
        return;
    }

    if (type === 'session.completed') {
        addCLILine('', 'plain');
        addCLILine('✅ 处理完成，结果已回传 QQ', 'success');
        addCLILine('━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'separator');
        addCLILine('', 'plain');
        return;
    }
    if (type === 'session.failed') {
        addCLILine('', 'plain');
        addCLILine('❌ 处理失败: ' + String(data?.error || 'unknown error'), 'error');
        addCLILine('━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'separator');
        addCLILine('', 'plain');
        return;
    }
    ...
    if (type === 'codex.event') {
        ...
        if (evt === 'thread.started') {
            const tid = data?.event?.thread_id || '';
            addCLILine(`🧵 线程启动 thread=${tid ? tid.slice(0, 12) : '-'}`, 'info');
            return;
        }
        if (evt === 'turn.started') {
            addCLILine('🔄 Codex 开始处理...', 'event');
            return;
        }
        if (evt === 'turn.completed') {
            const usage = data?.event?.usage;
            if (usage) {
                addCLILine(`🏁 处理结束  tokens: in=${usage.input_tokens || 0} out=${usage.output_tokens || 0}`, 'success');
            } else {
                addCLILine('🏁 处理结束', 'success');
            }
            return;
        }
        if (evt === 'turn.failed') {
            const errMsg = data?.event?.error?.message || 'unknown error';
            addCLILine(`❌ 处理失败: ${errMsg}`, 'error');
            return;
        }

        if (evt === 'item.started' && item) {
            if (item.type === 'command_execution') {
                addCLILine(`⚡ 执行命令: ${item.command || '(unknown)'}`, 'command');
                return;
            }
            if (item.type === 'file_change') {
                addCLILine('📝 文件修改开始...', 'command');
                return;
            }
            ...
            if (item.type === 'web_search') {
                addCLILine(`🔍 搜索: ${item.query || ''}`, 'event');
                return;
            }
            if (item.type === 'mcp_tool_call') {
                addCLILine(`🔧 MCP调用: ${item.server || ''}/${item.tool || ''}`, 'event');
                return;
            }
            if (item.type === 'todo_list') {
                addCLILine('📋 TODO 列表更新', 'info');
                return;
            }
            return;
        }

        if (evt === 'item.completed' && item) {
            if (item.type === 'command_execution') {
                const exitCode = item.exit_code ?? '-';
                const status = item.status || '';
                addCLILine(`   ↳ 命令完成 exit=${exitCode} (${status})`, exitCode === 0 ? 'success' : 'warning');
                const output = truncStr(item.aggregated_output || '', 500);
                if (output) {
            ...
            if (item.type === 'file_change' && Array.isArray(item.changes)) {
                for (const ch of item.changes) {
                    ...
                    const icon = kind === 'add' ? '➕' : kind === 'delete' ? '🗑️' : '✏️';
                    addCLILine(`   ${icon} ${kind}: ${ch.file || '(unknown)'}`, 'command');
                }
                return;
            }
            if (item.type === 'reasoning' && item.text) {
                const preview = truncStr(item.text, 300);
                addCLILine(`   💭 ${preview}`, 'info');
                return;
            }
            if (item.type === 'agent_message' && item.text) {
                addCLILine('📤 Codex 输出:', 'qq-out');
                const preview = truncStr(item.text, 500);
            ...
            if (item.type === 'web_search') {
                addCLILine(`   🔍 搜索完成: ${item.query || ''}`, 'success');
                return;
            }
            if (item.type === 'mcp_tool_call') {
                const st = item.error ? 'failed' : 'completed';
                addCLILine(`   🔧 MCP ${st}: ${item.server || ''}/${item.tool || ''}`, item.error ? 'error' : 'success');
                return;
            }
            if (item.type === 'todo_list' && Array.isArray(item.items)) {
                for (const todo of item.items) {
                    const check = todo.completed ? '✓' : '○';
                    addCLILine(`   ${check} ${todo.text || ''}`, 'info');
                }
                return;
            }
            if (item.type === 'error') {
                addCLILine(`   ⚠️ ${item.message || 'error'}`, 'error');
                return;
            }
            return;
        }

        if (evt === 'item.updated' && item) {
            if (item.type === 'command_execution' && item.aggregated_output) {
                const tail = tailLines(item.aggregated_output, 3);
                if (tail) {
    ...
    if (type === 'runtime.config.updated') {
        addCLILine(`⚙️ 配置已更新: model=${data?.runtime?.model || '-'} sandbox=${data?.runtime?.sandboxMode || '-'}`, 'system');
        return;
    }

    if (type.includes('error')) {
        addCLILine('❌ [bridge] ' + String(data?.message || type), 'error');
    }
}
...
function initCLITerminal(forceReset = false) {
    const terminal = document.getElementById('cli-terminal');
    if (!terminal) return;
    if (cliMonitorInitialized && !forceReset) return;

    cliMonitorInitialized = true;
    terminal.innerHTML = '<div class="text-slate-500 text-xs p-2">Initializing terminal...</div>';
    cliBuffer = [];
    currentLine = null;
    updateCLIStatus('Connecting', 'warning');
    updateCLISessionLabel(null);
    initCliXterm();

    addCLILine('========================================', 'header');
    addCLILine('  Codex CLI Live Monitor', 'header');
    addCLILine('  source: codexbridge /cli-stream', 'info');
    addCLILine('========================================', 'header');
    addCLILine('Connecting stream...', 'info');
    addCLILine('', 'plain');
}

function initCliXterm() {
    const el = document.getElementById('cli-terminal');
    if (!el) return;
    if (typeof Terminal === 'undefined') {
        cliXtermReady = false;
        addCLILine('[terminal] xterm.js unavailable, fallback to plain mode', 'warning');
        return;
    }
    if (cliXterm) return;
    el.innerHTML = '';
    cliXterm = new Terminal({
        cursorBlink: false,
        convertEol: false,
        disableStdin: true,
...
function addCLILine(text, style = 'plain') {
    if (cliXterm) {
        cliXterm.writeln(String(text ?? ''));
        return;
    }
    const terminal = document.getElementById('cli-terminal');
    if (!terminal) return;
    ...
    line.className = 'cli-line cli-' + style;
    ...
    line.style.color = colors[style] || colors.plain;
...
function trimCliBuffer() {
    while (cliBuffer.length > MAX_CLI_LINES) {
        const oldLine = cliBuffer.shift();
        if (oldLine === currentLine) currentLine = null;
...
function appendCLIText(text) {
    if (cliXterm) {
        cliXterm.write(String(text || ''));
        return;
    }
    const terminal = document.getElementById('cli-terminal');
    if (!terminal || !text) return;
    const clean = normalizeTerminalOutput(text);
    if (!clean) return;
    appendCLIPlainText(clean, terminal);
}

function ensureCLIPlainLine(terminal) {
    if (currentLine) return currentLine;
    currentLine = document.createElement('div');
    currentLine.className = 'cli-line cli-text';
    currentLine.style.color = '#e2e8f0';
    currentLine.style.fontFamily = 'Consolas, Monaco, monospace';
    currentLine.style.fontSize = '11px';
    currentLine.style.lineHeight = '1.5';
    currentLine.style.whiteSpace = 'pre-wrap';
    currentLine.style.wordBreak = 'break-word';
    terminal.appendChild(currentLine);
    cliBuffer.push(currentLine);
    trimCliBuffer();
    return currentLine;
}

function appendCLIPlainText(text, terminal) {
    let lineEl = ensureCLIPlainLine(terminal);
    for (const ch of String(text || '')) {
        if (ch === '\r') {
            // Typical CLI progress updates use CR to redraw current row.
            lineEl.textContent = '';
            continue;
        }
        if (ch === '\n') {
        ...
        if (ch === '\b') {
            if (lineEl && lineEl.textContent) {
                lineEl.textContent = lineEl.textContent.slice(0, -1);
            }
            continue;
        }
        if (!lineEl) {
            lineEl = ensureCLIPlainLine(terminal);
        }
...
function normalizeTerminalOutput(text) {
    ...
    // Keep CR/BS for plain-mode terminal emulation.
    out = out.replace(/\r\n/g, '\n');
    out = out.replace(/[\x00-\x07\x0B\x0C\x0E-\x1F\x7F]/g, '');
    return out;
}

function updateCLIStatus(status, type) {
...
function updateCLISessionLabel(sessionId) {
    const sessionEl = document.getElementById('cli-session-label');
    if (!sessionEl) return;
    sessionEl.textContent = sessionId ? `Source: ${sessionId}` : 'Source: idle';
}
...
function clearCliTerminal() {
    ...
    cliMonitorInitialized = false;
    initCLITerminal(true);
}

function parseCliBoolean(value) {
    const normalized = String(value || '').trim().toLowerCase();
    if (['on', 'true', '1', 'yes', 'y', 'enable', 'enabled'].includes(normalized)) return true;
    if (['off', 'false', '0', 'no', 'n', 'disable', 'disabled'].includes(normalized)) return false;
    throw new Error(`invalid boolean value "${value}", expected on/off`);
}

async function fetchRuntimeConfig(printToTerminal = false) {
    const response = await fetch('/api/runtime-config');
    const data = await response.json();
    if (!response.ok || data.error) {
        throw new Error(data?.error?.message || data?.error || 'failed to fetch runtime config');
    }
    const runtime = data.runtime || data;
    cliRuntimeConfigCache = runtime;
    if (printToTerminal) {
        printRuntimeConfig(runtime);
    }
    return runtime;
}

async function updateRuntimeConfig(updates) {
    const response = await fetch('/api/runtime-config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ updates })
    ...
    const data = await response.json();
    if (!response.ok || data.error) {
        throw new Error(data?.error?.message || data?.error || 'failed to update runtime config');
    }
    const runtime = data.runtime || data;
    cliRuntimeConfigCache = runtime;
    return runtime;
...
function printRuntimeConfig(runtime) {
    if (!runtime) return;
    addCLILine('[config] current runtime:', 'info');
    addCLILine(`  model: ${runtime.model}:${runtime.reasoning}`, 'info');
    addCLILine(`  sandbox: ${runtime.sandboxMode || 'default'}`, 'info');
    addCLILine(`  network: ${runtime.networkAccess ? 'on' : 'off'}`, 'info');
    addCLILine(`  websearch: ${runtime.webSearch ? 'on' : 'off'}`, 'info');
    addCLILine(`  approval: ${runtime.approvalPolicy || 'none'}`, 'info');
    addCLILine(`  workdir: ${runtime.workingDirectory || '(none)'}`, 'info');
}
...
navigateTo = function(viewId) {
    ...
    if (currentView === 'codex-cli' && viewId !== 'codex-cli') stopCodexCLIMonitor();
    ...
    else if (viewId === 'codex-cli') startCodexCLIMonitor();
